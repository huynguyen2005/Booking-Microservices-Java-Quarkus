...
export default function ProfilePage() {
  ...
  const uploadMut = useMutation({
    ...
    onSuccess: () => { toast.success('Avatar đã cập nhật!'); qc.invalidateQueries({ queryKey: ['me'] }); },
    onError: (e: any) => toast.error(e.response?.data?.message || e.response?.data || 'Upload thất bại'),
  });

  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => { const f = e.target.files?.[0]; if (f) uploadMut.mutate(f); };
    ...
    <div className="max-w-3xl mx-auto px-4 py-8 animate-fade-in">
      ...
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-lg)] p-6 sm:p-8 shadow-sm">
        <div className="flex flex-col sm:flex-row items-center gap-6 mb-8">
          <div className="relative">
            <div className="w-24 h-24 rounded-full bg-[var(--color-primary-soft)] flex items-center justify-center overflow-hidden border-2 border-[var(--color-primary)]">
              {u.avatarUrl ? <img src={u.avatarUrl} alt="" className="w-full h-full object-cover" /> : <UserIcon className="w-10 h-10 text-[var(--color-primary)]" />}
            </div>
            <button onClick={() => fileRef.current?.click()} className="absolute -bottom-1 -right-1 w-8 h-8 bg-[var(--color-primary)] text-white rounded-full flex items-center justify-center shadow-md hover:bg-[var(--color-primary-hover)] transition-colors">
        ...
        <div className="grid gap-4 text-sm">
          ...
          <div className="flex items-center gap-3 p-3 bg-[var(--color-surface-subtle)] rounded-md"><Phone className="w-4 h-4 text-[var(--color-text-muted)]" /><div><p className="text-xs text-[var(--color-text-muted)]">SĐT</p><p className="font-medium">{u.phone ?? 'Chưa cập nhật'}</p></div></div>
