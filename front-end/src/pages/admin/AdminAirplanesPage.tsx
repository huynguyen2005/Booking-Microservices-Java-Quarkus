...
export default function AdminAirplanesPage() {
  ...
  const openModal = (a?: Airplane) => {
    if (a) { setEditingId(a.id); setForm({ code: a.code ?? '', model: a.model ?? '', totalSeats: a.totalSeats }); }
    else { setEditingId(null); setForm({ code: '', model: '', totalSeats: 0 }); }
    setModalOpen(true);
  };

  const createMut = useMutation({ mutationFn: flightApi.createAirplane, onSuccess: () => { toast.success('Thêm máy bay thành công!'); setModalOpen(false); qc.invalidateQueries({ queryKey: ['adminAirplanes'] }); }, onError: (e: any) => toast.error(e.response?.data?.message || e.response?.data || 'Lỗi') });
  const updateMut = useMutation({ mutationFn: (d: { id: number; payload: Partial<Airplane> }) => flightApi.updateAirplane(d.id, d.payload), onSuccess: () => { toast.success('Cập nhật thành công!'); setModalOpen(false); qc.invalidateQueries({ queryKey: ['adminAirplanes'] }); }, onError: (e: any) => toast.error(e.response?.data?.message || e.response?.data || 'Lỗi') });
  const deleteMut = useMutation({ mutationFn: flightApi.deleteAirplane, onSuccess: () => { toast.success('Đã xóa!'); setDeleteId(null); qc.invalidateQueries({ queryKey: ['adminAirplanes'] }); }, onError: (e: any) => { toast.error(e.response?.data?.message || e.response?.data || 'Lỗi'); setDeleteId(null); } });
  const uploadMut = useMutation({ mutationFn: (file: File) => flightApi.uploadAirplaneImage(uploadId!, file), onSuccess: () => { toast.success('Upload ảnh thành công!'); setUploadId(null); qc.invalidateQueries({ queryKey: ['adminAirplanes'] }); }, onError: () => toast.error('Upload thất bại') });
    ...
    <div className="animate-fade-in">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'Sửa máy bay' : 'Thêm máy bay'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div><label className="block text-sm font-medium mb-1.5">Mã (VD: VN-A123) <span className="text-[var(--color-danger)]">*</span></label><input required value={form.code} onChange={e => setForm({ ...form, code: e.target.value })} className="uppercase" /></div>
          <div><label className="block text-sm font-medium mb-1.5">Model <span className="text-[var(--color-danger)]">*</span></label><input required value={form.model} onChange={e => setForm({ ...form, model: e.target.value })} /></div>
          <div><label className="block text-sm font-medium mb-1.5">Tổng số ghế <span className="text-[var(--color-danger)]">*</span></label><input required type="number" min="1" value={form.totalSeats} onChange={e => setForm({ ...form, totalSeats: Number(e.target.value) })} /></div>
