...
export default function AdminAirplanesPage() {
  ...
  const createMut = useMutation({
    mutationFn: flightApi.createAirplane,
    onSuccess: () => {
      toast.success('Thêm máy bay thành công');
      setModalOpen(false);
      qc.invalidateQueries({ queryKey: ['adminAirplanes'] });
    },
    onError: (e: any) => toast.error(e.response?.data?.message || e.response?.data || 'Có lỗi xảy ra'),
  });

  const updateMut = useMutation({
    mutationFn: (d: { id: number; payload: Partial<Airplane> }) => flightApi.updateAirplane(d.id, d.payload),
    onSuccess: () => {
  ...
  const deleteMut = useMutation({
    mutationFn: flightApi.deleteAirplane,
    onSuccess: () => {
      toast.success('Đã xóa máy bay');
      setDeleteId(null);
      qc.invalidateQueries({ queryKey: ['adminAirplanes'] });
    },
    onError: (e: any) => {
  ...
  const uploadMut = useMutation({
    mutationFn: (file: File) => flightApi.uploadAirplaneImage(uploadId!, file),
    onSuccess: () => {
  ...
  const columns: Column<Airplane>[] = useMemo(() => [
    ...
    { key: 'imageUrl', header: 'Ảnh', sortable: false, render: a => a.imageUrl ? <img src={a.imageUrl} alt="Airplane" className="w-10 h-10 rounded object-cover" /> : <ImageIcon className="w-5 h-5 opacity-30" /> },
    {
      key: '_actions',
      header: '',
      sortable: false,
  ...
  return (
    <div className="animate-fade-in">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'Cập nhật máy bay' : 'Thêm máy bay'}>
        <form onSubmit={e => { e.preventDefault(); if (editingId) updateMut.mutate({ id: editingId, payload: form }); else createMut.mutate(form); }} className="space-y-4">
          ...
          <div><label className="block text-sm font-medium mb-1.5">Tổng số ghế</label><input required type="number" min="1" value={form.totalSeats} onChange={e => setForm({ ...form, totalSeats: Number(e.target.value) })} /></div>
      ...
      <ConfirmDialog open={deleteId !== null} onClose={() => setDeleteId(null)} onConfirm={() => deleteId && deleteMut.mutate(deleteId)} title="Xóa máy bay" message="Bạn có chắc chắn muốn xóa máy bay này? Hành động này không thể hoàn tác." confirmText="Xóa" variant="danger" loading={deleteMut.isPending} />
    </div>
  );
}
