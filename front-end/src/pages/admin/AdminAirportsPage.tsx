...
export default function AdminAirportsPage() {
  ...
  const createMut = useMutation({ mutationFn: flightApi.createAirport, onSuccess: () => { toast.success('Thêm sân bay thành công'); setModalOpen(false); qc.invalidateQueries({ queryKey: ['adminAirports'] }); }, onError: (e: any) => toast.error(e.response?.data?.message || e.response?.data || 'Có l?i x?y ra') });
  const updateMut = useMutation({ mutationFn: (d: { id: number; payload: Partial<Airport> }) => flightApi.updateAirport(d.id, d.payload), onSuccess: () => { toast.success('C?p nh?t sân bay thành công'); setModalOpen(false); qc.invalidateQueries({ queryKey: ['adminAirports'] }); }, onError: (e: any) => toast.error(e.response?.data?.message || e.response?.data || 'Có l?i x?y ra') });
  const deleteMut = useMutation({ mutationFn: flightApi.deleteAirport, onSuccess: () => { toast.success('Ðã xóa sân bay'); setDeleteId(null); qc.invalidateQueries({ queryKey: ['adminAirports'] }); }, onError: (e: any) => { toast.error(e.response?.data?.message || e.response?.data || 'Có l?i x?y ra'); setDeleteId(null); } });
  const uploadMut = useMutation({ mutationFn: (file: File) => flightApi.uploadAirportImage(uploadId!, file), onSuccess: () => { toast.success('Upload ?nh thành công'); setUploadId(null); qc.invalidateQueries({ queryKey: ['adminAirports'] }); }, onError: () => toast.error('Upload th?t b?i') });
  ...
  const columns: Column<Airport>[] = useMemo(() => [
    ...
    { key: 'city', header: 'Thành ph?', render: a => a.city ?? '-' },
    { key: 'imageUrl', header: '?nh', sortable: false, render: a => a.imageUrl ? <img src={a.imageUrl} alt="Airport" className="w-10 h-10 rounded object-cover" /> : <ImageIcon className="w-5 h-5 opacity-30" /> },
    {
      key: '_actions',
      header: '',
      sortable: false,
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
        <h1 className="text-2xl font-bold text-[var(--color-text-main)]">Qu?n lý sân bay</h1>
        <div className="flex gap-2 items-center">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--color-text-muted)]" />
            <input className="pl-9 w-64" placeholder="Tìm mã/tên/thành ph?" value={keyword} onChange={e => setKeyword(e.target.value)} />
          </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'C?p nh?t sân bay' : 'Thêm sân bay'}>
        <form onSubmit={e => {
          e.preventDefault();
          ...
          <div><label className="block text-sm font-medium mb-1.5">Thành ph?</label><input required value={form.city} onChange={e => setForm({ ...form, city: e.target.value })} /></div>
      ...
      <ConfirmDialog open={deleteId !== null} onClose={() => setDeleteId(null)} onConfirm={() => deleteId && deleteMut.mutate(deleteId)} title="Xóa sân bay" message="B?n có ch?c ch?n mu?n xóa sân bay này?" confirmText="Xóa" variant="danger" loading={deleteMut.isPending} />
    </div>
  );
}
