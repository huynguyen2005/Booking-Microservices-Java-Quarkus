...
export default function MyPassengersPage() {
  ...
  const openModal = (p?: Passenger) => {
    if (p) { setEditingId(p.id); setForm({ fullName: p.fullName ?? '', email: p.email ?? '', phone: p.phone ?? '', passportNumber: p.passportNumber ?? '' }); }
    else { setEditingId(null); setForm({ fullName: '', email: '', phone: '', passportNumber: '' }); }
    setModalOpen(true);
  };

  const createMut = useMutation({ mutationFn: passengerApi.createPassenger, onSuccess: () => { toast.success('Thêm hành khách thành công!'); setModalOpen(false); qc.invalidateQueries({ queryKey: ['myPassengers'] }); }, onError: (e: any) => toast.error(e.response?.data?.message || e.response?.data || 'Lỗi') });
  const updateMut = useMutation({ mutationFn: (d: { id: number; payload: Partial<Passenger> }) => passengerApi.updatePassenger(d.id, d.payload), onSuccess: () => { toast.success('Cập nhật thành công!'); setModalOpen(false); qc.invalidateQueries({ queryKey: ['myPassengers'] }); }, onError: (e: any) => toast.error(e.response?.data?.message || e.response?.data || 'Lỗi') });
  const deleteMut = useMutation({ mutationFn: passengerApi.deletePassenger, onSuccess: () => { toast.success('Đã xóa!'); setDeleteId(null); qc.invalidateQueries({ queryKey: ['myPassengers'] }); }, onError: (e: any) => { toast.error(e.response?.data?.message || e.response?.data || 'Lỗi'); setDeleteId(null); } });
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 animate-fade-in">
      <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--color-text-muted)]" /><input className="pl-9 w-52 text-sm" placeholder="Tìm kiếm..." value={keyword} onChange={e => setKeyword(e.target.value)} /></div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-lg)]">
          ...
          <p className="text-sm text-[var(--color-text-muted)] mt-1 mb-4">Thêm hành khách để đặt vé cho họ.</p>
          <Button onClick={() => openModal()}>+ Thêm hành khách</Button>
        </div>
      ) : (
        <div className="grid gap-3">
          {passengers.map(p => (
            <div key={p.id} className="flex items-center justify-between p-4 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-md)] hover:shadow-sm transition-all">
              <div>
                <p className="font-semibold text-[var(--color-text-main)]">{p.fullName ?? 'Không tên'}</p>
                <p className="text-sm text-[var(--color-text-muted)]">{[p.email, p.phone, p.passportNumber].filter(Boolean).join(' • ') || 'Chưa cập nhật'}</p>
              </div>
